...
// in attacker-controlled ways. We approximate it by giving the fuzzer independent string inputs, each seeded
...
// Pair an attacker-chosen (subject, signer) and run VerifyCertSignature. The seeds are role-agnostic
// because a real attack tries every cross-pairing — a NOC signed by another NOC, an RCAC subject with an
...
FUZZ_TEST(FuzzChipCert, ValidateChipNetworkIdentityFuzz)
    .WithDomains(Arbitrary<std::string>().WithSeeds(seedProvider(isChipFile)));
...
// attacker-shaped-but-validly-signed input.
//
...
// attacker-controlled bytes: the ECDSA signature raw<->DER conversion sits on attestation-response and
