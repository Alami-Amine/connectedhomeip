...
std::string BuildAttestationElementsSeed()
{
    ...
    if (ConstructAttestationElements(ByteSpan(certDecl), ByteSpan(nonce), 0xDEADBEEF,
                                      ByteSpan(firmwareInfo), vendorReservedCtor, out) != CHIP_NO_ERROR)
    {
...
std::string BuildCertificationDeclarationSeed(uint8_t pidCount = 1, bool dacOrigin = false,
                                              uint8_t paaCount = 0, uint8_t certType = 0,
                                              uint16_t versionNumber = 0x2694)
{
    Platform::MemoryInit();
...
void DeconstructAttestationFuzz(const std::string & bytes)
{
    ...
    RETURN_SAFELY_IGNORED DeconstructAttestationElements(span, certDecl, attestationNonce, timestamp,
                                                         firmwareInfo, vendorReserved);
