...
// FuzzTest harness for the WebRTCTransportProvider cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::SolicitOffer::DecodableType>(bytes);
    DriveDecode<Commands::ProvideOffer::DecodableType>(bytes);
    DriveDecode<Commands::ProvideAnswer::DecodableType>(bytes);
    DriveDecode<Commands::ProvideICECandidates::DecodableType>(bytes);
    DriveDecode<Commands::EndSession::DecodableType>(bytes);
}
FUZZ_TEST(WebRTCTransportProvider, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::SolicitOffer::Type>(),
        EncSeed<Commands::ProvideOffer::Type>(),
        EncSeed<Commands::ProvideAnswer::Type>(),
        EncSeed<Commands::ProvideICECandidates::Type>(),
        EncSeed<Commands::EndSession::Type>(),
    }));

void StructDecode(const std::vector<std::uint8_t> & bytes)
{
    DriveDecode<Globals::Structs::ICEServerStruct::DecodableType>(bytes);
    DriveDecode<Globals::Structs::ICECandidateStruct::DecodableType>(bytes);
    DriveDecode<Structs::SFrameStruct::DecodableType>(bytes);
}
FUZZ_TEST(WebRTCTransportProvider, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Globals::Structs::ICEServerStruct::Type>(),
        EncSeed<Globals::Structs::ICECandidateStruct::Type>(),
        EncSeed<Structs::SFrameStruct::Type>(),
    }));
