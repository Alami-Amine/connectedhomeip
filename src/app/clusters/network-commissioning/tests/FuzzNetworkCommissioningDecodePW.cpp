...
// FuzzTest harness for the NetworkCommissioning cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::ScanNetworks::DecodableType>(bytes);
    DriveDecode<Commands::AddOrUpdateWiFiNetwork::DecodableType>(bytes);
    DriveDecode<Commands::AddOrUpdateThreadNetwork::DecodableType>(bytes);
    DriveDecode<Commands::RemoveNetwork::DecodableType>(bytes);
    DriveDecode<Commands::ConnectNetwork::DecodableType>(bytes);
    DriveDecode<Commands::ReorderNetwork::DecodableType>(bytes);
    DriveDecode<Commands::QueryIdentity::DecodableType>(bytes);
}
FUZZ_TEST(NetworkCommissioning, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::ScanNetworks::Type>(),
        EncSeed<Commands::AddOrUpdateWiFiNetwork::Type>(),
        EncSeed<Commands::AddOrUpdateThreadNetwork::Type>(),
        EncSeed<Commands::RemoveNetwork::Type>(),
        EncSeed<Commands::ConnectNetwork::Type>(),
        EncSeed<Commands::ReorderNetwork::Type>(),
        EncSeed<Commands::QueryIdentity::Type>(),
    }));

void StructDecode(const std::vector<std::uint8_t> & bytes)
{
    DriveDecode<Structs::NetworkInfoStruct::DecodableType>(bytes);
    DriveDecode<Structs::WiFiInterfaceScanResultStruct::DecodableType>(bytes);
    DriveDecode<Structs::ThreadInterfaceScanResultStruct::DecodableType>(bytes);
}
FUZZ_TEST(NetworkCommissioning, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Structs::NetworkInfoStruct::Type>(),
        EncSeed<Structs::WiFiInterfaceScanResultStruct::Type>(),
        EncSeed<Structs::ThreadInterfaceScanResultStruct::Type>(),
    }));
