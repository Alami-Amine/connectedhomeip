...
// FuzzTest harness for the AdministratorCommissioning cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::OpenCommissioningWindow::DecodableType>(bytes);
    DriveDecode<Commands::OpenBasicCommissioningWindow::DecodableType>(bytes);
    DriveDecode<Commands::RevokeCommissioning::DecodableType>(bytes);
}
FUZZ_TEST(AdministratorCommissioning, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::OpenCommissioningWindow::Type>(),
        EncSeed<Commands::OpenBasicCommissioningWindow::Type>(),
        EncSeed<Commands::RevokeCommissioning::Type>(),
    }));
