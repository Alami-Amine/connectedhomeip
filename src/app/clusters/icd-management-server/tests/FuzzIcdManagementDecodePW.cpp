...
// FuzzTest harness for the IcdManagement cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::RegisterClient::DecodableType>(bytes);
    DriveDecode<Commands::UnregisterClient::DecodableType>(bytes);
    DriveDecode<Commands::StayActiveRequest::DecodableType>(bytes);
}
FUZZ_TEST(IcdManagement, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::RegisterClient::Type>(),
        EncSeed<Commands::UnregisterClient::Type>(),
        EncSeed<Commands::StayActiveRequest::Type>(),
    }));

void StructDecode(const std::vector<std::uint8_t> & bytes)
{
    DriveDecode<Structs::MonitoringRegistrationStruct::DecodableType>(bytes);
}
FUZZ_TEST(IcdManagement, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Structs::MonitoringRegistrationStruct::Type>(),
    }));
