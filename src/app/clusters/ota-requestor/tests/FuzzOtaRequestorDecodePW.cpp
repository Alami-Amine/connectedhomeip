...
// FuzzTest harness for the OtaSoftwareUpdateRequestor cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
// The Requestor decodes the Provider's response commands (QueryImageResponse / ApplyUpdateResponse)
// from the wire, so pull in the Provider command definitions too.
...
{
    DriveDecode<OtaSoftwareUpdateRequestor::Commands::AnnounceOTAProvider::DecodableType>(bytes);
    DriveDecode<OtaSoftwareUpdateProvider::Commands::QueryImageResponse::DecodableType>(bytes);
    DriveDecode<OtaSoftwareUpdateProvider::Commands::ApplyUpdateResponse::DecodableType>(bytes);
}
FUZZ_TEST(OtaSoftwareUpdateRequestor, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<OtaSoftwareUpdateRequestor::Commands::AnnounceOTAProvider::Type>(),
        EncSeed<OtaSoftwareUpdateProvider::Commands::QueryImageResponse::Type>(),
        EncSeed<OtaSoftwareUpdateProvider::Commands::ApplyUpdateResponse::Type>(),
    }));

void StructDecode(const std::vector<std::uint8_t> & bytes)
{
    DriveDecode<OtaSoftwareUpdateRequestor::Structs::ProviderLocation::DecodableType>(bytes);
}
FUZZ_TEST(OtaSoftwareUpdateRequestor, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<OtaSoftwareUpdateRequestor::Structs::ProviderLocation::Type>(),
    }));
