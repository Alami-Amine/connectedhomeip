...
// FuzzTest harness for the GroupKeyManagement cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::KeySetWrite::DecodableType>(bytes);
    DriveDecode<Commands::KeySetRead::DecodableType>(bytes);
    DriveDecode<Commands::KeySetRemove::DecodableType>(bytes);
    DriveDecode<Commands::KeySetReadAllIndices::DecodableType>(bytes);
    DriveDecode<Commands::KeySetReadAllIndicesResponse::DecodableType>(bytes);
}
FUZZ_TEST(GroupKeyManagement, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::KeySetWrite::Type>(),
        EncSeed<Commands::KeySetRead::Type>(),
        EncSeed<Commands::KeySetRemove::Type>(),
        EncSeed<Commands::KeySetReadAllIndices::Type>(),
        EncSeed<Commands::KeySetReadAllIndicesResponse::Type>(),
    }));

void StructDecode(const std::vector<std::uint8_t> & bytes)
{
    DriveDecode<Structs::GroupKeySetStruct::DecodableType>(bytes);
    DriveDecode<Structs::GroupInfoMapStruct::DecodableType>(bytes);
    DriveDecode<Structs::GroupKeyMapStruct::DecodableType>(bytes);
}
FUZZ_TEST(GroupKeyManagement, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Structs::GroupKeySetStruct::Type>(),
        EncSeed<Structs::GroupInfoMapStruct::Type>(),
        EncSeed<Structs::GroupKeyMapStruct::Type>(),
    }));
