...
// FuzzTest harness for the DoorLock cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::LockDoor::DecodableType>(bytes);
    DriveDecode<Commands::UnlockDoor::DecodableType>(bytes);
    DriveDecode<Commands::UnlockWithTimeout::DecodableType>(bytes);
    DriveDecode<Commands::UnboltDoor::DecodableType>(bytes);
    DriveDecode<Commands::SetWeekDaySchedule::DecodableType>(bytes);
    DriveDecode<Commands::SetYearDaySchedule::DecodableType>(bytes);
    DriveDecode<Commands::SetHolidaySchedule::DecodableType>(bytes);
    DriveDecode<Commands::SetUser::DecodableType>(bytes);
    DriveDecode<Commands::SetCredential::DecodableType>(bytes);
    DriveDecode<Commands::GetCredentialStatus::DecodableType>(bytes);
    DriveDecode<Commands::ClearCredential::DecodableType>(bytes);
    DriveDecode<Commands::SetAliroReaderConfig::DecodableType>(bytes);
}
FUZZ_TEST(DoorLock, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::LockDoor::Type>(),
        EncSeed<Commands::UnlockDoor::Type>(),
        EncSeed<Commands::UnlockWithTimeout::Type>(),
        EncSeed<Commands::UnboltDoor::Type>(),
        EncSeed<Commands::SetWeekDaySchedule::Type>(),
        EncSeed<Commands::SetYearDaySchedule::Type>(),
        EncSeed<Commands::SetHolidaySchedule::Type>(),
        EncSeed<Commands::SetUser::Type>(),
        EncSeed<Commands::SetCredential::Type>(),
        EncSeed<Commands::GetCredentialStatus::Type>(),
        EncSeed<Commands::ClearCredential::Type>(),
        EncSeed<Commands::SetAliroReaderConfig::Type>(),
    }));

void StructDecode(const std::vector<std::uint8_t> & bytes)
{
    DriveDecode<Structs::CredentialStruct::DecodableType>(bytes);
}
FUZZ_TEST(DoorLock, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Structs::CredentialStruct::Type>(),
    }));
