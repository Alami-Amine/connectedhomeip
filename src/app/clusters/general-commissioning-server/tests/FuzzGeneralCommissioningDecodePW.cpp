...
// FuzzTest harness for the GeneralCommissioning cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::ArmFailSafe::DecodableType>(bytes);
    DriveDecode<Commands::SetRegulatoryConfig::DecodableType>(bytes);
    DriveDecode<Commands::CommissioningComplete::DecodableType>(bytes);
    DriveDecode<Commands::SetTCAcknowledgements::DecodableType>(bytes);
}
FUZZ_TEST(GeneralCommissioning, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::ArmFailSafe::Type>(),
        EncSeed<Commands::SetRegulatoryConfig::Type>(),
        EncSeed<Commands::CommissioningComplete::Type>(),
        EncSeed<Commands::SetTCAcknowledgements::Type>(),
    }));
