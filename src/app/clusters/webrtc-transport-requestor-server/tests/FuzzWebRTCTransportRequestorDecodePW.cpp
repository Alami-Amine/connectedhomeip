...
// FuzzTest harness for the WebRTCTransportRequestor cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::Offer::DecodableType>(bytes);
    DriveDecode<Commands::Answer::DecodableType>(bytes);
    DriveDecode<Commands::ICECandidates::DecodableType>(bytes);
    DriveDecode<Commands::End::DecodableType>(bytes);
}
FUZZ_TEST(WebRTCTransportRequestor, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::Offer::Type>(),
        EncSeed<Commands::Answer::Type>(),
        EncSeed<Commands::ICECandidates::Type>(),
        EncSeed<Commands::End::Type>(),
    }));
...
FUZZ_TEST(WebRTCTransportRequestor, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
