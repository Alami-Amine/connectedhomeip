...
// FuzzTest harness for the ScenesManagement cluster TLV DECODE paths (wire-reachable, attacker-controlled
...
{
    DriveDecode<Commands::AddScene::DecodableType>(bytes);
    DriveDecode<Commands::ViewScene::DecodableType>(bytes);
    DriveDecode<Commands::RemoveScene::DecodableType>(bytes);
    DriveDecode<Commands::RemoveAllScenes::DecodableType>(bytes);
    DriveDecode<Commands::StoreScene::DecodableType>(bytes);
    DriveDecode<Commands::RecallScene::DecodableType>(bytes);
    DriveDecode<Commands::GetSceneMembership::DecodableType>(bytes);
    DriveDecode<Commands::CopyScene::DecodableType>(bytes);
}
FUZZ_TEST(ScenesManagement, CommandDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Commands::AddScene::Type>(),
        EncSeed<Commands::ViewScene::Type>(),
        EncSeed<Commands::RemoveScene::Type>(),
        EncSeed<Commands::RemoveAllScenes::Type>(),
        EncSeed<Commands::StoreScene::Type>(),
        EncSeed<Commands::RecallScene::Type>(),
        EncSeed<Commands::GetSceneMembership::Type>(),
        EncSeed<Commands::CopyScene::Type>(),
    }));

void StructDecode(const std::vector<std::uint8_t> & bytes)
{
    DriveDecode<Structs::ExtensionFieldSetStruct::DecodableType>(bytes);
    DriveDecode<Structs::AttributeValuePairStruct::DecodableType>(bytes);
    DriveDecode<Structs::SceneInfoStruct::DecodableType>(bytes);
}
FUZZ_TEST(ScenesManagement, StructDecode)
    .WithDomains(Arbitrary<std::vector<std::uint8_t>>().WithSeeds({
        EncSeed<Structs::ExtensionFieldSetStruct::Type>(),
        EncSeed<Structs::AttributeValuePairStruct::Type>(),
        EncSeed<Structs::SceneInfoStruct::Type>(),
    }));
