...
def list_fuzz_test_binaries():
    ...
    for root, _, files in os.walk(build_dir):
        for file in files:
            if file.startswith("fuzz-") and os.access(os.path.join(root, file), os.X_OK):
...
def main(fuzz_test, output):
    ...
    if not fuzz_tests:
        logging.info("No fuzz tests found.")
        return
    ...
    if test_cases:
        ...
        if 1 <= choice <= len(test_cases):
            ...
            coverage_output_base_name = f"{selected_fuzz_test_case.replace(".", "_")}"
