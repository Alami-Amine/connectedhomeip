...
def extract_build_target_name(binary_path):
    """Extract build directory from the path by finding directory above 'chip_pw_fuzztest'."""
    parts = binary_path.split(os.sep)
    build_target_name = "default"

    for i, part in enumerate(parts):
        if part == "chip_pw_fuzztest" and i > 0:
            build_target_name = parts[i-1]
            break
...
def get_fuzz_test_cases(context):
    ...
    try:
        ...
        result = subprocess.run([context.fuzz_test_binary_path, "--list_fuzz_tests"], capture_output=True, text=True)
