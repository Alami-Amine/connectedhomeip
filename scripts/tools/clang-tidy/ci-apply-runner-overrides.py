...
the runner itself, so the PR clang-tidy job is unaffected. Rationale (matter-clang-tidy
runbook):

  -Wno-error               The compile commands carry -Werror. Warnings our from-source
                           clang-tidy emits but the original build does not
  ...
  --header-filter=src/     Frontend checks (bugprone-*) do NOT display a finding whose
                           location is a header unless this is set; much chip::Optional
                           code is inline in headers/templates.
